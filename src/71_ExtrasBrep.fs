...
module ExtrasBrep = 
    ...
    let filletArc  (prevPt:Point3d, midPt:Point3d, nextPt:Point3d, radius:float)  : Arc   = 
        let A = midPt-prevPt
        let B = midPt-nextPt
        let uA = A |> unitize 
        let uB = B |> unitize      
    ...
    let filletPolyline (fillets: ResizeArray<int*float>, polyline:IList<Point3d>): PolyCurve = 
        let R=dict()
        ...
        if R.ContainsKey 0 then
            if closed then 
                let arc = filletArc (polyline.Last, polyline.[0], polyline.[1], R.[0])
        ...
        for i = 1 to polyline.Count - 2 do  
            ...
            if R.ContainsKey i then 
                ...
                let arc = filletArc (prevPt, pt, ptn, R.[i])
    ...
    let filletSkewLinesTrims (radius:float) (direction:Vector3d) (lineA:Line) (lineB:Line): float  =         
        let ok,axis = 
    ...
    let filletSkewLines makeSCurve (radius:float)  (direction:Vector3d) (lineA:Line) (lineB:Line): NurbsCurve*Arc*Point3d   = 
        let ok,axis = 
           let pla = Plane(lineA.From, lineA.Direction, direction)
           let plb = Plane(lineB.From, lineB.Direction, direction)            
           Intersect.Intersection.PlanePlane(pla,plb)
