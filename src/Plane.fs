...
module ExtensionsPlane =
  ...
  type RhinoScriptSyntax with
    ...
    static member PlaneCurveIntersection(plane:Plane, curve:Guid, [<OPT;DEF(0.0)>]tolerance:float) : float * Point3d * Point3d * Point3d * Point3d * float * float * float * float * float =
        failNotImpl () // genreation temp disabled !!
    ...
    ///<returns>(Plane) The plane .</returns>
    static member PlaneFromNormal(origin:Point3d, normal:Vector3d, [<OPT;DEF(Vector3d())>]xaxis:Vector3d) : Plane =
        failNotImpl () // genreation temp disabled !!
