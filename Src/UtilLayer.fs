...
module internal UtilLayer =
    ...
    let randomLayerColor()=
        ...
        let r,g,b = ColorUtil.fromHSL(h,s,l)
    ...
    let internal failOnBadShortLayerName(name:string, fullPath:string, allowAllUnicode) : unit=
        ...
        if name.Contains "::" then
            RhinoScriptingException.Raise "%s: The layer name '%s' shall not contain two colons (::) in '%s'." eVSLN name fullPath
    ...
    let internal lockedSetFalse(lay:Layer,  parentsToo:bool) : unit =
        if lay.IsLocked then
            // Note: Setting lay.IsLocked <- false directly fails with message box if parents are locked
            if parentsToo then
