  ...
  type RhinoScriptSyntax with
    ...
    static member AddBlock(objectIds:Guid seq, basePoint:Point3d, [<OPT;DEF("")>]name:string, [<OPT;DEF(false)>]deleteInput:bool) : string =
        ...
        for objectId in objectIds do
            ...
            if obj.IsReference then  RhinoScriptingException.Raise "RhinoScriptSyntax.AddBlock: cannt add Refrence Object %A to %s" objectId name
            let ot = obj.ObjectType
            if   ot= DocObjects.ObjectType.Light then  RhinoScriptingException.Raise "RhinoScriptSyntax.AddBlock: cannot add Light Object %A to %s" objectId name
            elif ot= DocObjects.ObjectType.Grip then  RhinoScriptingException.Raise "RhinoScriptSyntax.AddBlock: cannot add Grip Object %A to %s" objectId name
            elif ot= DocObjects.ObjectType.Phantom then RhinoScriptingException.Raise "RhinoScriptSyntax.AddBlock: cannot add Phantom Object %A to %s" objectId name
                ...
                if uses then RhinoScriptingException.Raise "RhinoScriptSyntax.AddBlock: cannt add Instance Ref Object %A to %s" objectId name
