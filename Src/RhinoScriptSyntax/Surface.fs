  ...
  //[<Extension>] //Error 3246
    ...
    static member AddCutPlane( objectIds:Guid seq,
        ...
        if not bbox.IsValid then
            RhinoScriptingException.Raise "RhinoScriptSyntax.AddCutPlane failed.  objectIds:'%A' startPoint:'%A' endPoint:'%A' normal:'%A'" objectIds startPoint endPoint normal
        ...
        if surface|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.AddCutPlane failed.  objectIds:'%A' startPoint:'%A' endPoint:'%A' normal:'%A'" objectIds startPoint endPoint normal
        let objectId = Doc.Objects.AddSurface(surface)
        if objectId = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.AddCutPlane failed.  objectIds:'%A' startPoint:'%A' endPoint:'%A' normal:'%A'" objectIds startPoint endPoint normal
        Doc.Views.Redraw()
        objectId
    ...
    static member AddEdgeSrf(curveIds:Guid seq) : Guid =
        ...
        if brep|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.AddEdgeSrf failed.  curveIds:'%A'" curveIds
        let objectId = Doc.Objects.AddBrep(brep)
        if objectId = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.AddEdgeSrf failed.  curveIds:'%A'" curveIds
        Doc.Views.Redraw()
        objectId
    ...
    static member AddPatch( objectIds:Guid seq,
                    ...
                    if notNull brep then
                        ...
                        RhinoScriptingException.Raise "RhinoScriptSyntax.AddPatch faild for %A and %A " objectIds startSurfaceId
    ...
    static member AddPatch( objectIds:Guid seq,
        ...
        if notNull brep then
            ...
            RhinoScriptingException.Raise "RhinoScriptSyntax.AddPatch faild for %A and %A " objectIds uvSpans
    ...
    static member AddPlanarSrf(objectIds:Guid seq) : Guid Rarr =
        ...
        if notNull breps then
            ...
            RhinoScriptingException.Raise "RhinoScriptSyntax.AddPlanarSrf faild on %A" objectIds
    ...
    static member AddLoftSrf( objectIds:Guid seq,
        ...
        if loftType<0 || loftType>4 then RhinoScriptingException.Raise "RhinoScriptSyntax.Rhino.Scripting.AddLoftSrf: LoftType must be 0-4.  objectIds:'%A' start:'%A' end:'%A' loftType:'%A' rebuild:'%A' refit:'%A' closed:'%A'" objectIds start ende loftType rebuild refit closed
        if rebuild<>0 && refit<>0.0 then RhinoScriptingException.Raise "RhinoScriptSyntax.Rhino.Scripting.AddLoftSrf: set either rebuild or refit to a value ! not both.  objectIds:'%A' start:'%A' end:'%A' loftType:'%A' rebuild:'%A' refit:'%A' closed:'%A'" objectIds start ende loftType rebuild refit closed
        let curves =  rarr { for objectId in objectIds do yield RhinoScriptSyntax.CoerceCurve(objectId) }
        if Seq.length(curves)<2 then RhinoScriptingException.Raise "RhinoScriptSyntax.AddLoftSrf failed.  objectIds:'%A' start:'%A' end:'%A' loftType:'%A' rebuild:'%A' refit:'%A' closed:'%A'" objectIds start ende loftType rebuild refit closed
        ...
        if isNull breps then RhinoScriptingException.Raise "RhinoScriptSyntax.AddLoftSrf failed.  objectIds:'%A' start:'%A' end:'%A' loftType:'%A' rebuild:'%A' refit:'%A' closed:'%A'" objectIds start ende loftType rebuild refit closed
        let idlist = Rarr()
    ...
    static member AddRevSrf( curveId:Guid,
        ...
        if isNull srf then RhinoScriptingException.Raise "RhinoScriptSyntax.AddRevSrf failed.  curveId:'%A' axis:'%A' startAngle:'%A' endAngle:'%A'" curveId axis startAngle endAngle
        let ns = srf.ToNurbsSurface()
        if isNull ns then RhinoScriptingException.Raise "RhinoScriptSyntax.AddRevSrf failed.  curveId:'%A' axis:'%A' startAngle:'%A' endAngle:'%A'" curveId axis startAngle endAngle
    ...
    static member BrepClosestPoint(objectId:Guid, point:Point3d) : Point3d * float * float * ComponentIndexType * int * Vector3d =
        ...
        if ok then
            ...
            RhinoScriptingException.Raise "RhinoScriptSyntax.BrepClosestPoint faile for %A and %A" objectId point
    ...
    static member DuplicateSurfaceBorder(surfaceId:Guid, [<OPT;DEF(0)>]typ:int) : Guid Rarr =
        ...
        if curves|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.DuplicateSurfaceBorder failed.  surfaceId:'%A' typ:'%A'" surfaceId typ
    ...
    static member EvaluateSurface( surfaceId:Guid,
        ...
        else RhinoScriptingException.Raise "RhinoScriptSyntax.EvaluateSurface failed.  surfaceId:'%A' u:'%A' v:'%A'" surfaceId u v
    ...
    static member ExtrudeCurve(curveId:Guid, pathId:Guid) : Guid =
        ...
        if rc = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.ExtrudeCurve failed.  curveId:'%A' pathId:'%A'" curveId pathId
        Doc.Views.Redraw()
        rc
    ...
    static member ExtrudeCurvePoint(curveId:Guid, point:Point3d) : Guid =
        ...
        let rc = Doc.Objects.AddSurface(srf)
        if rc = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.ExtrudeCurvePoint failed.  curveId:'%A' point:'%A'" curveId point
        Doc.Views.Redraw()
        rc
    ...
    static member ExtrudeCurveStraight( curveId:Guid,
        ...
        let rc = Doc.Objects.AddSurface(srf)
        if rc = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.ExtrudeCurveStraight failed.  curveId:'%A' startPoint:'%A' endPoint:'%A'" curveId startPoint endPoint
        Doc.Views.Redraw()
        rc
    ...
    static member FlipSurface(surfaceId:Guid) : bool = //GET
        let brep = RhinoScriptSyntax.CoerceBrep(surfaceId)
        if brep.Faces.Count>1 then RhinoScriptingException.Raise "RhinoScriptSyntax.FlipSurface Get failed.  surfaceId:'%A' " surfaceId
    ...
    static member FlipSurface(surfaceId:Guid, flip:bool) : unit = //SET
        let brep = RhinoScriptSyntax.CoerceBrep(surfaceId)
        if brep.Faces.Count>1 then RhinoScriptingException.Raise "RhinoScriptSyntax.FlipSurface failed.  surfaceId:'%A' flip:'%A'" surfaceId flip
        let face = brep.Faces.[0]
        let oldreverse = face.OrientationIsReversed
    ...
    static member IsPointInSurface( objectId:Guid,
        ...
        //point = RhinoScriptSyntax.Coerce3dpoint(point)
        //if objectId|> isNull  || point|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.IsPointInSurface failed.  objectId:'%A' point:'%A' strictlyIn:'%A' tolerance:'%A'" objectId point strictlyIn tolerance
    ...
    static member IsPointOnSurface(objectId:Guid, point:Point3d) : bool =
        ...
        if rc  then
            ...
            RhinoScriptingException.Raise "RhinoScriptSyntax.IsPointOnSurface failed for surf.ClosestPoint on %A %A" objectId point
        rc
    ...
    static member JoinSurfaces(objectIds:Guid seq, [<OPT;DEF(false)>]deleteInput:bool) : Guid =
        ...
        if breps.Count<2 then RhinoScriptingException.Raise "RhinoScriptSyntax.JoinSurfaces failed, less than two objects given.  objectIds:'%A' deleteInput:'%A'" objectIds deleteInput
        let tol = Doc.ModelAbsoluteTolerance * 2.1
        let joinedbreps = Brep.JoinBreps(breps, tol)
        if joinedbreps|> isNull  then
            RhinoScriptingException.Raise "RhinoScriptSyntax.JoinSurfaces failed.  objectIds:'%A' deleteInput:'%A'" objectIds deleteInput
        ...
        if rc = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.JoinSurfaces failed.  objectIds:'%A' deleteInput:'%A'" objectIds deleteInput
        if  deleteInput then
            for objectId in objectIds do
    ...
    static member MakeSurfacePeriodic( surfaceId:Guid,
        ...
        if newsurf|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.MakeSurfacePeriodic failed.  surfaceId:'%A' direction:'%A' deleteInput:'%A'" surfaceId direction deleteInput
    ...
    static member OffsetSurface( surfaceId:Guid,
        ...
        if face|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.OffsetSurface failed.  surfaceId:'%A' distance:'%A' tolerance:'%A' bothSides:'%A' createSolid:'%A'" surfaceId distance tolerance bothSides createSolid
        let tolerance= ifZero1 tolerance Doc.ModelAbsoluteTolerance
        let newbrep = Brep.CreateFromOffsetFace(face, distance, tolerance, bothSides, createSolid)
        if newbrep|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.OffsetSurface failed.  surfaceId:'%A' distance:'%A' tolerance:'%A' bothSides:'%A' createSolid:'%A'" surfaceId distance tolerance bothSides createSolid
        let rc = Doc.Objects.AddBrep(newbrep)
        if rc = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.OffsetSurface failed.  surfaceId:'%A' distance:'%A' tolerance:'%A' bothSides:'%A' createSolid:'%A'" surfaceId distance tolerance bothSides createSolid
        Doc.Views.Redraw()
        rc
    ...
    static member ReverseSurface(surfaceId:Guid, direction:int) : unit =
        let brep = RhinoScriptSyntax.CoerceBrep(surfaceId)
        if brep.Faces.Count <> 1 then RhinoScriptingException.Raise "RhinoScriptSyntax.ReverseSurface failed.  surfaceId:'%A' direction:'%A'" surfaceId direction
        let face = brep.Faces.[0]
    ...
    static member ShortPath( surfaceId:Guid,
        ...
        if not rcstart || not rcend then RhinoScriptingException.Raise "RhinoScriptSyntax.ShortPath failed.  surfaceId:'%A' startPoint:'%A' endPoint:'%A'" surfaceId startPoint endPoint
        let start = Point2d(ustart, vstart)
        let ende = Point2d(uend, vend)
        let tolerance = Doc.ModelAbsoluteTolerance
        let curve = surface.ShortPath(start, ende, tolerance)
        if curve|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.ShortPath failed.  surfaceId:'%A' startPoint:'%A' endPoint:'%A'" surfaceId startPoint endPoint
        let objectId = Doc.Objects.AddCurve(curve)
        if objectId = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.ShortPath failed.  surfaceId:'%A' startPoint:'%A' endPoint:'%A'" surfaceId startPoint endPoint
        Doc.Views.Redraw()
        objectId
    ...
    static member ShrinkTrimmedSurface(objectId:Guid, [<OPT;DEF(false)>]createCopy:bool) : Guid =
        let brep = RhinoScriptSyntax.CoerceBrep(objectId)
        if brep.Faces.ShrinkFaces() then RhinoScriptingException.Raise "RhinoScriptSyntax.ShrinkTrimmedSurface failed.  objectId:'%A' createCopy:'%A'" objectId createCopy
        if  createCopy then
    ...
    static member SplitBrep( brepId:Guid,
        ...
        if isNull pieces then RhinoScriptingException.Raise "RhinoScriptSyntax.SplitBrep failed.  brepId:'%A' cutterId:'%A' deleteInput:'%A'" brepId cutterId deleteInput
        if  deleteInput then
    ...
    static member SurfaceAreaCentroid(objectId:Guid) : Point3d =
        ...
        |> Option.defaultWith (fun () -> RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceAreaCentroid failed.  objectId:'%A'" objectId )
    ...
    static member SurfaceCone(surfaceId:Guid) : Plane * float * float =
        ...
        if not rc then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceCone failed.  surfaceId:'%A'" surfaceId
    ...
    static member SurfaceCurvature(surfaceId:Guid, parameter:float * float) : Point3d * Vector3d * float * Vector3d * float * Vector3d * float * float=
        let surface = RhinoScriptSyntax.CoerceSurface(surfaceId)
        //if Seq.length(parameter)<2 then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceCurvature failed.  surfaceId:'%A' parameter:'%A'" surfaceId parameter
        let c = surface.CurvatureAt(parameter|> fst, parameter|> snd)
        if c|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceCurvature failed.  surfaceId:'%A' parameter:'%A'" surfaceId parameter
    ...
    static member SurfaceDomain(surfaceId:Guid, direction:int) : float * float =
        if direction <> 0 && direction <> 1 then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceDomain failed.  surfaceId:'%A' direction:'%A'" surfaceId direction
        let surface = RhinoScriptSyntax.CoerceSurface(surfaceId)
    ...
    static member SurfaceEditPoints( surfaceId:Guid, [<OPT;DEF(true)>]returnAll:bool) : Point3d Rarr =
        ...
        if isNull nurb then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceEditPoints failed.  surfaceId:'%A'  returnAll:'%A'" surfaceId returnAll
    ...
    static member SurfaceEditPointPrameters( surfaceId:Guid, [<OPT;DEF(true)>]returnAll:bool) : (float*float) Rarr =
        let surface = RhinoScriptSyntax.CoerceSurface(surfaceId)
        let nurb = surface.ToNurbsSurface()
        if isNull nurb then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceEditPointParameterss failed.  surfaceId:'%A'  returnAll:'%A'" surfaceId returnAll
        let mutable ufirst = 0
        let mutable ulast = nurb.Points.CountU
        let mutable vfirst = 0
        let mutable vlast = nurb.Points.CountV
    ...
    static member SurfaceEvaluate( surfaceId:Guid,
        ...
        if not success then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceEvaluate failed.  surfaceId:'%A' parameter:'%A' derivative:'%A'" surfaceId parameter derivative
        let rc = rarr{()}
    ...
    static member SurfaceIsocurveDensity(surfaceId:Guid) : int = //GET
        ...
        | :?  DocObjects.ExtrusionObject as rhinoobject ->
                rhinoobject.Attributes.WireDensity
        | _ -> RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceIsocurveDensity Get failed.  surfaceId:'%A' " surfaceId
    ...
    static member SurfaceIsocurveDensity(surfaceId:Guid, density:int) : unit = //SET
        ...
        | :?  DocObjects.ExtrusionObject as rhinoobject ->
                ...
                rhinoobject.CommitChanges() |> RhinoScriptingException.FailIfFalse "CommitChanges failed" 
                Doc.Views.Redraw()
        | _ -> RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceIsocurveDensity Get failed.  surfaceId:'%A' density:'%A'" surfaceId density
    ...
    static member SurfaceKnots(surfaceId:Guid) : Collections.NurbsSurfaceKnotList * Collections.NurbsSurfaceKnotList=
        ...
        if nurbsurf|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceKnots failed.  surfaceId:'%A'" surfaceId
        ...
        //if isNull sknots || not tknots then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceKnots failed.  surfaceId:'%A'" surfaceId
        //sknots, tknots
    ...
    static member SurfaceNormalizedParameter(surfaceId:Guid, parameter:float * float) : float * float =
        ...
        if parameter|> fst<udomain.Min || parameter|> fst>udomain.Max then
            RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceNormalizedParameter failed.  surfaceId:'%A' parameter:'%A'" surfaceId parameter
    ...
    static member SurfacePoints(surfaceId:Guid, [<OPT;DEF(true)>]returnAll:bool) : Point3d Rarr =
        let surface = RhinoScriptSyntax.CoerceSurface(surfaceId)
        let ns = surface.ToNurbsSurface()
        if ns|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfacePoints failed.  surfaceId:'%A' returnAll:'%A'" surfaceId returnAll
        let rc = Rarr()
    ...
    static member SurfaceVolume(objectId:Guid) : float =
        ...
        |> Option.defaultWith (fun () -> RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceVolume failed on %A" objectId)
        |> fun amp -> amp.Volume
    ...
    static member SurfaceVolumeCentroid(objectId:Guid) : Point3d =
        objectId
        |> RhinoScriptSyntax.TryCoerceBrep
        |> Option.bind (fun b -> if b.IsSolid then Some b else RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceVolumeCentroid failed on  open Brep %A" objectId)
        |> Option.map VolumeMassProperties.Compute
        |> Option.orElseWith (fun () ->
            objectId
            |> RhinoScriptSyntax.TryCoerceSurface
            |> Option.bind (fun s -> if s.IsSolid then Some s else RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceVolumeCentroid failed on  open Surface %A" objectId)
            |> Option.map VolumeMassProperties.Compute
            )
        |> Option.defaultWith (fun () -> RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceVolumeCentroid failed on %A" objectId)
        |> fun amp -> amp.Centroid
    ...
    static member SurfaceVolumeMoments(objectId:Guid) : (float*float*float) Rarr =
        objectId
        |> RhinoScriptSyntax.TryCoerceBrep
        |> Option.bind (fun b -> if b.IsSolid then Some b else RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceVolumeMoments failed on  open Brep %A" objectId)
        |> Option.map VolumeMassProperties.Compute
        |> Option.orElseWith (fun () ->
            objectId
            |> RhinoScriptSyntax.TryCoerceSurface
            |> Option.bind (fun s -> if s.IsSolid then Some s else RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceVolumeMoments failed on  open Surface %A" objectId)
            |> Option.map VolumeMassProperties.Compute
            )
        |> Option.defaultWith (fun () -> RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceVolumeMoments failed on %A" objectId)
        |> fun mp ->
            rarr{
                yield (mp.WorldCoordinatesFirstMoments.X, mp.WorldCoordinatesFirstMoments.Y, mp.WorldCoordinatesFirstMoments.Z)                                     //  [0]     First Moments.
                yield (mp.WorldCoordinatesFirstMomentsError.X, mp.WorldCoordinatesFirstMomentsError.Y, mp.WorldCoordinatesFirstMomentsError.Z)                      //  [1]     The absolute (+/-) error bound for the First Moments.
    ...
    static member SurfaceWeights(objectId:Guid) : float Rarr =
        let surface = RhinoScriptSyntax.CoerceSurface(objectId)
        let ns = surface.ToNurbsSurface()
        if ns|> isNull  then RhinoScriptingException.Raise "RhinoScriptSyntax.SurfaceWeights failed.  objectId:'%A'" objectId
        let rc = Rarr()
        for u in range(ns.Points.CountU) do
            for v in range(ns.Points.CountV) do
                let pt = ns.Points.GetControlPoint(u, v)
    ...
    static member ChangeSurfaceDegree(objectId:Guid, degree:int * int) : bool =
        ...
        if u < 1 || u > maxnurbsdegree || v < 1 || v > maxnurbsdegree ||  (surface.Degree(0) = u && surface.Degree(1) = v) then RhinoScriptingException.Raise "RhinoScriptSyntax.ChangeSurfaceDegree failed on %A" objectId
        let mutable r = false
