...
type [<Struct>] Quat = 
    ...
    private new (x, y, z, w) = // assume  unitized ??   // make private ?? 
        if abs(x*x+y*y+z*z+w*w-1.0) > 1e-9 then failwithf "*** Quat constructors are not unitized: %g %g %g %g" x y z w // skip check ??
    ...
    static member fromVecRad (axis:Vector3d, angleRadian) =
        ...
        if li < 1e-8 then failwithf "*** Cannot create Quat to rotate %g radians around zero length vector %A" angleRadian axis //or return identity ?
